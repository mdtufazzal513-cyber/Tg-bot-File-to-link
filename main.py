...
def generate_link(user_id, views, expire_hours, chat_id, title="Untitled"):
    ...
    # ১. টেলিগ্রাম শেয়ারিং লিংক
    tg_link = f"https://t.me/{bot.get_me().username}?start={unique_id}"
    
    # Render-এর আসল লিংকটি এখানে বসান (যেমন: https://viral-bot.onrender.com)
    # Render নিজে থেকেও এটি বুঝতে পারে, তবে ম্যানুয়ালি দিয়ে রাখা ভালো।
    render_url = os.environ.get("RENDER_EXTERNAL_URL", "https://tg-bot-file-to-link.onrender.com")
    
    msg_text = f"🎉 <b>আপনার লিংক তৈরি হয়েছে!</b>\n\n📝 <b>Title:</b> {title}\n\n"
    
    if db_msg_id:
        # ২. ডিরেক্ট স্ট্রিমিং লিংক (ব্রাউজারে প্লে করার জন্য)
        stream_link = f"{render_url}/watch/{db_msg_id}"
        
        # ৩. ডিরেক্ট ডাউনলোড লিংক (ডাউনলোড বাটনের জন্য)
        dl_link = f"{render_url}/watch/{db_msg_id}?download=true"
        
        msg_text += f"▶️ <b>স্ট্রিমিং লিংক (Web):</b>\n<code>{stream_link}</code>\n\n"
        msg_text += f"📥 <b>ডাউনলোড লিংক:</b>\n<code>{dl_link}</code>\n\n"
    else:
...
async def stream_handler(request):
    try:
        ...
        if media_type == "photo":
            ...
            # NoneType এরর ফিক্স করা হয়েছে
            file_name = getattr(media, "file_name", None)
            if not file_name:  # টেলিগ্রাম নাম না দিলে আমরা একটি ডিফল্ট নাম দিয়ে দিব
            ...
            # ফাইলের এক্সটেনশন ঠিক করা
            if media_type == "video" and not file_name.endswith('.mp4'):
                file_name += ".mp4"
            elif media_type == "audio" and not file_name.endswith(('.mp3', '.m4a')):
                file_name += ".mp3"
