...
def handle_files(message):
    ...
    # ফাইলের আসল নাম বের করে সেভ করা হচ্ছে
    if message.document: 
        file_id, file_type, file_name = message.document.file_id, "document", getattr(message.document, "file_name", "document.file")
    elif message.video: 
        file_id, file_type, file_name = message.video.file_id, "video", getattr(message.video, "file_name", "video.mp4")
    elif message.audio: 
        file_id, file_type, file_name = message.audio.file_id, "audio", getattr(message.audio, "file_name", "audio.mp3")
    elif message.photo: 
        file_id, file_type, file_name = message.photo[-1].file_id, "photo", f"photo_{message.message_id}.jpg"
    elif message.animation: 
        file_id, file_type, file_name = message.animation.file_id, "document", getattr(message.animation, "file_name", "animation.mp4")
    elif message.text: 
        file_id, file_type, file_name = message.text, "text", "text" 

    if not file_name:
        file_name = f"unknown_{message.message_id}.bin"
...
async def stream_handler(request):
    try:
        ...
        # মিডিয়ার ধরন অনুযায়ী ফাইলের নাম এবং mime_type ফিক্স করা
        if media_type == "photo":
            ...
            if media_type == "video":
                if not file_name.endswith('.mp4'):
                    file_name += ".mp4"
                # ভিডিও হলে ব্রাউজার যেন ডাউনলোড না করে প্লে করে তাই mime_type ভিডিও করে দেওয়া হলো
                if "octet-stream" in mime_type or "matroska" in mime_type:
                    mime_type = "video/mp4"
                    
            elif media_type == "audio":
                if not file_name.endswith(('.mp3', '.m4a')):
                    file_name += ".mp3"
