...
def generate_link(user_id, views, expire_hours, chat_id, title="Untitled"):
    ...
    if db_channel_check and file_storage[unique_id]['files'][0].get('db_msg_id'):
        ...
        for idx, f in enumerate(file_storage[unique_id]['files']):
            ...
            if f_db_id:
                msg_text += f"📄 <b>ফাইল {idx+1}:</b>\n"
                msg_text += f"▶️ <b>Stream:</b> <code>{render_url}/watch/{f_db_id}</code>\n"
                msg_text += f"📥 <b>Download:</b> <code>{render_url}/watch/{f_db_id}?download=true</code>\n\n"
    else:
