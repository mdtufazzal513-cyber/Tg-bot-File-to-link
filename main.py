...
def get_admin_reply_menu():
    ...
    markup.add(KeyboardButton("📊 Statistics"), KeyboardButton("👥 Manage Users"))
    markup.add(KeyboardButton("🔗 Manage Links"), KeyboardButton("🖥 System Stats"))
...
def generate_link(user_id, views, expire_hours, chat_id, title="Untitled"):
    ...
    # চেক করছি ফাইলটি ড্রপবক্স থেকে এসেছে কি না
    is_dropbox = file_storage[unique_id]['files'][0].get('is_dropbox', False)
    
    if db_channel_check and file_storage[unique_id]['files'][0].get('db_msg_id') and is_dropbox:
        msg_text += "🌐 <b>ডিরেক্ট ওয়েব লিংকসমূহ (For Web/App):</b>\n\n"
        # লুপ চালিয়ে সব ফাইলের লিংক জেনারেট করা হবে
        for idx, f in enumerate(file_storage[unique_id]['files']):
            f_db_id = f.get('db_msg_id')
            if f_db_id:
                # সেভ করা অরিজিনাল নাম আনা হচ্ছে
                saved_name = f.get('name', f"file_{f_db_id}.mp4")
                # লিংকের ভেতর নাম বসানোর জন্য নিরাপদ ফরমেটে কনভার্ট করা (যেমন স্পেস থাকলে %20 হবে)
                from urllib.parse import quote
                safe_name = quote(saved_name)
                
                msg_text += f"📄 <b>ফাইল {idx+1}:</b> {saved_name}\n"
                # লিংকের একদম শেষে ফাইলের নাম (.mp4 সহ) যুক্ত করে দেওয়া হলো
