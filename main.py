...
def generate_link(user_id, views, expire_hours, chat_id, title="Untitled"):
    ...
    if db_channel_check:
        # DB Channel থাকলে স্ট্রিমিং লিংক দেখাবে
        ...
        for idx, f in enumerate(file_storage[unique_id]['files']):
            ...
            if f_db_id:
                ...
                f_size_readable = humanbytes(f.get('size', 0))
                msg_text += f"📄 <b>ফাইল {idx+1}:</b> {saved_name}\n"
                msg_text += f"📊 <b>Size:</b> <code>{f_size_readable}</code>\n"
                msg_text += f"▶️ <b>Stream:</b> <code>{render_url}/watch/{f_db_id}/{safe_name}</code>\n"
        
        if not has_streaming:
            msg_text += "⚠️ <i>(ফাইলগুলো ডাটাবেসে সেভ না হওয়ায় স্ট্রিমিং লিংক তৈরি করা যায়নি!)</i>\n\n"
    else:
        # DB Channel সেট করা না থাকলে এই মেসেজ আসবে
...
def handle_dropbox_messages(message):
    ...
    db_channel = str(settings.get("DB_CHANNEL_ID", "")).strip()
    if not db_channel:
        return bot.send_message(message.chat.id, "⚠️ <b>আগে Settings থেকে DB Channel সেট করুন!</b>", parse_mode="HTML")
