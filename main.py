...
def cleanup_input(message):
    """ইউজারের ইনপুট এবং বটের আগের প্রশ্ন ডিলিট করে"""
    try:
        bot.delete_message(message.chat.id, message.message_id)
        # যদি ইউজার বটের মেসেজে রিপ্লাই দিয়ে থাকে বা বট রিপ্লাই চেয়ে থাকে
        if message.reply_to_message:
            bot.delete_message(message.chat.id, message.reply_to_message.message_id)
    except:
        # যদি রিপ্লাই না থাকে তবে শেষ পাঠানো মেসেজ ডিলিট করার চেষ্টা (অপশনাল)
        pass

# ================= অ্যাডমিন মেনু ও সেটিংস রেসপন্স =================
@bot.callback_query_handler(func=lambda call: True)
def handle_callbacks(call):
    # ইউজার ল্যাঙ্গুয়েজ চেক
...
def update_setting(message, key, val_type):
    ...
    if message.text.startswith('/'): return bot.send_message(message.chat.id, "❌ সেটিং আপডেট বাতিল করা হয়েছে।")
    try:
        val = message.text.strip()
        if key == "DB_CHANNEL_ID" and val != "0" and not val.startswith("-"):
            return bot.reply_to(message, "❌ <b>ভুল ID!</b> ডাটাবেস চ্যানেলের আইডি অবশ্যই <b>-100</b> দিয়ে শুরু হতে হবে।", parse_mode="HTML")
            
        if key in["FORCE_SUB_CHANNELS", "DB_CHANNEL_ID"] and val == "0": val = ""
        settings[key] = val_type(val)
        save_data(SETTINGS_FILE, settings)
        send_temp_message(message.chat.id, f"✅ <b>{key}</b> আপডেট করা হয়েছে। চেক করতে /start দিন।")
...
def process_delete_link(message):
    ...
    if link_id in file_storage:
        del file_storage[link_id]
        save_data(DATA_FILE, file_storage)
        send_temp_message(message.chat.id, f"✅ লিংক <b>{link_id}</b> সফলভাবে ডিলিট করা হয়েছে!")
    else:
...
def generate_link(user_id, views, expire_hours, chat_id, title="Untitled"):
    # টাইটেল দেওয়ার পর ইনপুটগুলো ডিলিট করার জন্য একটি ডামি মেসেজ অবজেক্ট তৈরি (যদি প্রয়োজন হয়)
    # যেহেতু টাইটেল ইনপুট থেকে সরাসরি এই ফাংশনে আসে, আমরা এখানে ক্লিনআপ করবো না,
    # বরং টাইটেল ইনপুট যেখানে নেওয়া হয় সেখানে cleanup_input(message) কল করবো।
    ...
    # ডাটাবেস চ্যানেল এবং Message ID সংগ্রহ করা
    ...
    # Render-এর আসল লিংকটি
    render_url = os.environ.get("RENDER_EXTERNAL_URL", "https://tg-bot-file-to-link.onrender.com")
    
    msg_text = f"🎉 <b>আপনার লিংক তৈরি হয়েছে!</b>\n\n📝 <b>Title:</b> {title}\n\n"
    
    db_channel_check = str(settings.get("DB_CHANNEL_ID", "")).strip()
    
    # ডাটাবেস চ্যানেল সেট করা থাকলে স্ট্রিমিং লিংক তৈরি হবে
    if db_channel_check and db_channel_check != "0":
        ...
        for idx, f in enumerate(file_storage[unique_id]['files']):
            ...
            if f_db_id:
                ...
                f_size = humanbytes(f.get('size', 0)) # সাইজ দেখাবে
        ...
        if not has_stream:
            msg_text += "⚠️ <i>(ফাইলগুলো ডাটাবেসে পাওয়া যায়নি, তাই স্ট্রিমিং লিংক তৈরি করা যায়নি!)</i>\n\n"
    else:
        msg_text += "⚠️ <i>(স্ট্রিমিং লিংক তৈরি হয়নি কারণ DB Channel সেট করা নেই!)</i>\n\n"
...
def handle_reply_keyboard(message):
    ...
    if text == "🌐 Language":
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("🇧🇩 বাংলা", callback_data="setlang_bn"),
    ...
    if text == "📤 Upload File":
        ...
        bot.send_message(message.chat.id, "📥 <b>দয়া করে Dropbox এর শেয়ারিং লিংকটি (যেমন: https://www.dropbox.com/s/...) মেসেজে লিখে পাঠান:</b>", parse_mode="HTML")
...
def handle_files(message):
    ...
    # 🌟 যদি মেসেজে ড্রপবক্স লিংক থাকে, তবে সেটি ড্রপবক্স ফাংশনে পাঠিয়ে দিবে 🌟
    ...
    if message.text and (message.text.startswith('/') or message.text in ["📣 Updates Channel", "📤 Upload File", "📥 Upload from Dropbox", "📊 Statistics", "🖥 System Stats", "👥 Manage Users", "🔗 Manage Links", "✉️ Send Notice", "⚙️ Settings", "📞 Contact Owner", "📢 Our Channel", "📝 Request File/Movie"]):
        return
    ...
    # টেলিগ্রাম থেকে নাম না পেলে ডিফল্ট .mp4 বা সঠিক এক্সটেনশন বসানো হবে
    if message.document: 
        file_id, file_type = message.document.file_id, "document"
        file_name = message.document.file_name if message.document.file_name else f"file_{message.message_id}.mp4"
    elif message.video: 
        file_id, file_type = message.video.file_id, "video"
        file_name = message.video.file_name if message.video.file_name else f"video_{message.message_id}.mp4"
    elif message.audio: 
        file_id, file_type = message.audio.file_id, "audio"
        file_name = message.audio.file_name if message.audio.file_name else f"audio_{message.message_id}.mp3"
    elif message.photo: 
        file_id, file_type = message.photo[-1].file_id, "photo"
        file_name = f"photo_{message.message_id}.jpg"
    elif message.animation: 
        file_id, file_type = message.animation.file_id, "document"
        file_name = message.animation.file_name if message.animation.file_name else f"animation_{message.message_id}.mp4"
    elif message.text: 
        file_id, file_type, file_name = message.text, "text", "text"

    db_msg_id = None
    db_channel = str(settings.get("DB_CHANNEL_ID", "")).strip()
    
    # ডাটাবেস আইডি ভ্যালিড কি না তা নিশ্চিত করা
    ...
    if file_id:
        ...
        # ডাটাবেসে ফাইলের নাম ও সাইজ সহ সেভ করা হচ্ছে
        user_temp_files[user_id_str].append({
            'id': file_id, 
            'type': file_type, 
            'db_msg_id': db_msg_id, 
            'name': file_name,
        ...
        # আগের পাঠানো বাটনের মেসেজটি ডিলিট করার চেষ্টা (যাতে বারবার মেনু না আসে)
        ...
        sent_status = bot.send_message(
            message.chat.id, 
            f"📦 <b>ফাইল রিসিভ হয়েছে!</b>\n\n📄 নাম: <code>{file_name}</code>\n📊 মোট ফাইল: <b>{len(user_temp_files[user_id_str])}</b>\n\nসব পাঠানো শেষ হলে নিচের যেকোনো একটি বাটনে ক্লিক করুন 👇", 
            reply_markup=markup, 
            parse_mode="HTML"
        )
        # নতুন মেসেজ আইডি সেভ করে রাখা হচ্ছে যাতে পরের বার ডিলিট করা যায়
        user_temp_files[f"{user_id_str}_last_msg"] = sent_status.message_id

# ================= DROPBOX DOWNLOADER SYSTEM =================
def process_dropbox_link_thread(message, db_channel, dropbox_url):
    try:
        ...
        if "?dl=1" not in direct_url and "?raw=1" not in direct_url:
            direct_url += "?raw=1"

        # লিংকের শেষ অংশ থেকে ফাইলের আসল নাম বের করা
        parsed_url = urllib.parse.urlparse(direct_url)
        file_name = parsed_url.path.split('/')[-1]
        if not file_name or "." not in file_name:
            file_name = f"dropbox_video_{message.message_id}.mp4"
        else:
        ...
        # ফাইল ডাউনলোড করা (Progress Bar সহ)
        with requests.get(direct_url, stream=True) as r:
            ...
            with open(local_filename, 'wb') as f:
                for chunk in r.iter_content(chunk_size=1024*1024): # 1MB Chunk
                    ...
                    # প্রতি ২ সেকেন্ড পর পর প্রগ্রেস বার আপডেট করবে (যাতে স্প্যাম না হয়)
                    if time.time() - last_update_time > 2:
                        progress = get_progress_bar(downloaded, total_size)
                        try:
                            bot.edit_message_text(f"⏳ <b>ডাউনলোড হচ্ছে...</b>\n\n{progress}\n📥 {humanbytes(downloaded)} / {humanbytes(total_size)}", message.chat.id, status_msg.message_id, parse_mode="HTML")
                        except: pass
                        last_update_time = time.time()
        
        bot.edit_message_text("✅ <b>ডাউনলোড শেষ! ডাটাবেস চ্যানেলে আপলোড হচ্ছে...</b>", message.chat.id, status_msg.message_id, parse_mode="HTML")
        
        # Pyrogram দিয়ে DB Channel এ আপলোড
        async def upload_task():
            try:
                client = clients[0]
                caption = f"DROPBOX_URL: {direct_url}"
                target_chat = int(str(db_channel).replace(" ", ""))
                
                # Peer Warm-up: চ্যাটটি আগে লোড করে নেওয়া
                try: await client.get_chat(target_chat)
                except: pass
                
                msg = await client.send_document(target_chat, local_filename, caption=caption, file_name=file_name)
                return msg.id
            except Exception as e:
                print(f"Upload Task Error: {e}")
                raise e

        future = asyncio.run_coroutine_threadsafe(upload_task(), STREAM_LOOP)
        db_msg_id = future.result()

        user_id_str = str(message.from_user.id)
        if user_id_str not in user_temp_files:
            user_temp_files[user_id_str] = []
        
        # is_dropbox = True সেট করা হলো যাতে ওয়েব লিংক তৈরি হয়
        ...
        bot.send_message(message.chat.id, f"🎉 <b>Dropbox ফাইল ডাটাবেসে সেভ হয়েছে!</b>\n\n📄 <b>Name:</b> {file_name}\n\nমোট ফাইল: {len(user_temp_files[user_id_str])}", reply_markup=markup, parse_mode="HTML")

    except Exception as e:
        bot.send_message(message.chat.id, f"❌ <b>Error:</b> {e}", parse_mode="HTML")
    finally:
...
def handle_dropbox_messages(message):
    ...
    db_channel = str(settings.get("DB_CHANNEL_ID", "")).strip()
    if not db_channel or db_channel == "0":
        return bot.send_message(message.chat.id, "⚠️ <b>স্ট্রিমিং ফিচারের জন্য আগে Settings থেকে DB Channel ID সেট করুন!</b>", parse_mode="HTML")
...
# ⚠️ এখানে শুধু ডামি বটগুলোর টোকেন কমা দিয়ে দিন (মেইন বটের TOKEN এখানে থাকবে না)
MULTI_CLIENT_TOKENS =[
    "8703236011:AAEA3279_ak38POI_TAVK0b9tKZVe_0fBN8",  # ডামি বট 1
    "8430400718:AAHpjC4R07SrHCO-6-J8ZMT2P8LcMarpm8k",  # ডামি বট 2
    "8711817641:AAGYG1DACABDKYgxxSPrSudm4BJnXcw999U"   # ডামি বট 3
]
...
async def stream_handler(request):
    try:
        ...
        if not db_channel_raw or str(db_channel_raw) == "0":
        ...
        try:
            ...
            # Peer Warm-up: ডামি বটগুলো যেন চ্যানেলটিকে চিনে নেয়
            try: await client.get_chat(target_db)
            except: pass
            
            message = await client.get_messages(chat_id=target_db, message_ids=message_id)
        except Exception as e:
            return web.Response(text=f"Error accessing DB Channel: {e}", status=500)
        
        # মেসেজ বা মিডিয়া চেক করা
        if not message or getattr(message, "empty", False):
            return web.Response(text="File not found! (Make sure dummy bots are Admins in DB Channel)", status=404)
            
        if not getattr(message, "media", None):
            return web.Response(text="No media found in this message!", status=404)
            
        # সঠিক মিডিয়া বের করা
        media_type = message.media.value # 'photo', 'video', 'document', 'audio'
        media = getattr(message, media_type)
        file_size = getattr(media, "file_size", 0)
        
        # লিংক থেকে সরাসরি অরিজিনাল ফাইলের নাম রিসিভ করা (যদি থাকে)
        from urllib.parse import unquote
        url_file_name = request.match_info.get('file_name')
        if url_file_name:
            file_name = unquote(url_file_name)
        else:
            file_name = getattr(media, "file_name", None)
            if not file_name:
                file_name = f"file_{message_id}.mp4"
        
        # মিডিয়ার ধরন অনুযায়ী mime_type ফিক্স করা
        if media_type == "photo":
            ...
            # ভিডিওর ক্ষেত্রে ব্রাউজার প্লেয়ার চালু করতে ফোর্স করা
            if media_type == "video" or file_name.endswith('.mp4') or file_name.endswith('.mkv'):
                if "octet-stream" in mime_type or "matroska" in mime_type:
                    mime_type = "video/mp4"
                    
            elif media_type == "audio" or file_name.endswith('.mp3'):
                if "octet-stream" in mime_type:
                    mime_type = "audio/mpeg"

        # 🌟 Dropbox লিংক এক্সট্রাক্ট করা 🌟
        dropbox_link = None
        if getattr(message, "caption", None) and "DROPBOX_URL:" in message.caption:
            ...
            if match:
                dropbox_link = match.group(1)

        is_download = request.query.get("download", "").lower() in ["true", "1", "yes"]
        
        # ডাউনলোড না হলে এবং ড্রপবক্স লিংক থাকলে ডিরেক্ট স্ট্রিমিংয়ে রিডাইরেক্ট (For App iFrame)
        if not is_download and dropbox_link:
            raise web.HTTPFound(dropbox_link)

        disp = "attachment" if is_download else "inline"
        safe_name = quote(file_name) # ফাইলের নামের স্পেস ফিক্স

        # 🚀 Advanced Headers for Perfect File Naming in Android/AppCreator24
        headers = {
        ...
        # ভিডিও টেনে দেখার জন্য (206) এবং নরমাল ডাউনলোডের জন্য (200) আলাদা লজিক
        if range_header:
            range_match = range_header.replace('bytes=', '').split('-')
            start = int(range_match[0])
            end = int(range_match[1]) if len(range_match) > 1 and range_match[1] else file_size - 1
            
            # রেঞ্জ যদি ফাইলের সাইজ থেকে বেশি হয় (ভিডিও প্লেয়ারের এরর এড়াতে)
            if start >= file_size:
        ...
        offset = start
        try:
            # টেলিগ্রাম থেকে ডাটা এনে ব্রাউজারে পুশ করা হচ্ছে
            async for chunk in client.stream_media(message, offset=offset, limit=chunk_size):
                await response.write(chunk)
                # ডাটা পাঠানোর পর সার্ভারকে একটু শ্বাস নেওয়ার সুযোগ দেওয়া (স্ট্যাবল কানেকশনের জন্য)
                await asyncio.sleep(0.001) 
        except (ConnectionResetError, BrokenPipeError):
            # ব্রাউজার নিজে থেকে কানেকশন কেটে দিলে ইগনোর করবে
            pass
        except Exception as e:
            pass # অন্যান্য এরর বাইপাস
...
async def run_web_and_clients():
    ...
    # পুরনো লিংকগুলোর জন্য
    app.router.add_get('/watch/{message_id}', stream_handler)  
    # নতুন লিংক (নাম ও .mp4 সহ) প্লে করার জন্য
...
# আগের কোনো জ্যাম থাকলে ক্লিয়ার করার জন্য
try:
    bot.remove_webhook()
    time.sleep(2)
except Exception as e:
...
while True:
    try:
        # Conflict এড়াতে non_stop False করা হলো
        bot.polling(non_stop=True, timeout=60, long_polling_timeout=30)
    except Exception as e:
        print(f"Telebot Error: {e}")
        time.sleep(10) # এরর আসলে ১০ সেকেন্ড অপেক্ষা করে আবার চেষ্টা করবে